(() => {
  ...
  // js/custom-element/behavior/collapsible-content.js
  var Customization = class extends OpenableElement {
    constructor() {
      super();
      var html = this.querySelector(".product-customization__form").innerHTML
      this.ignoreNextTransition = this.open;
      this.querySelector("#customization").addEventListener("change", () => {
        if (this.querySelector("#customization").checked) {
          this.open = true
          const font = this.querySelector(".product-customization__form").querySelector('input[type=radio]');
          font.checked = true;   
        } else {
          this.open = false
          this.querySelector(".product-customization__form").innerHTML = html
        }
      });
    }
    attributeChangedCallback(name) {
      ...
      switch (name) {
        case "open":
          this.querySelector(".product-customization__form").style.overflow = "hidden";
          const keyframes = {
            height: ["0px", `${this.querySelector(".product-customization__form").scrollHeight}px`],
            visibility: ["hidden", "visible"]
          };
          this.querySelector(".product-customization__form").animate(keyframes, {
            duration: 500,
            direction: this.open ? "normal" : "reverse",
            easing: "cubic-bezier(0.75, 0, 0.175, 1)"
          }).onfinish = () => {
            this.querySelector(".product-customization__form").style.overflow = this.open ? "visible" : "hidden";
          }   
      }
    }
  };
  window.customElements.define("customization-form", Customization);
...
`;
  ...
  // js/helper/product-loader-locale.js
  var ProductLoaderLocale = class {
    static load(productHandle) {
      if (!productHandle) {
        return;
      }
      if (this.loadedProductsLocale[productHandle]) {
        return this.loadedProductsLocale[productHandle];
      }
      this.loadedProductsLocale[productHandle] = new Promise(async (resolve) => {
  ...
  __publicField(ProductLoaderLocale, "loadedProductsLocale", {});
  ...
  var ProductMedia = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this.product = await ProductLoaderLocale.load(this.getAttribute("product-handle"));
      (_a = document.getElementById(this.getAttribute("form-id"))) == null ? void 0 : _a.addEventListener("variant:changed", this._onVariantChanged.bind(this));
          ...
          if (!matchMedia2) {
            ...
            this.product["options"].forEach((option) => {
              if (option["name"].toLowerCase() === mediaGroupParts[0].toLowerCase()) {
                if (this.product["variants"].find((item) => item["id"] === event.detail.variant["id"])["options"][option["position"] - 1].toLowerCase() !== mediaGroupParts[1].trim().toLowerCase()) {
  ...
  // js/custom-element/section/product/back-in-stock.js
  var BackInStock = class extends HTMLElement {
    connectedCallback() {
      var _a;
      (_a = document.getElementById(this.getAttribute("form-id"))) == null ? void 0 : _a.addEventListener("variant:changed", this._onVariantChanged.bind(this));
    }
    ...
    async _renderForVariant(id) {
      const response = await fetch(`${window.themeVariables.routes.rootUrlWithoutSlash}/variants/${id}?section_id=back-in-stock`), div = document.createElement("div");
      div.innerHTML = await response.text();
      this.innerHTML = div.firstElementChild.innerHTML.trim();
    }
  };
  ...
  // js/custom-element/section/product/back-in-stock-form.js
  var BackInStockForm = class extends HTMLFormElement {
    connectedCallback() {
      this.addEventListener("submit", this._onSubmit.bind(this));
    }
    async _onSubmit(event) {
      ...
      const backInStockForm = new FormData(this);
      backInStockForm.append("a", "W8sXMH");
      backInStockForm.append("email", document.getElementById("back-in-stock").value);
      backInStockForm.append("variant", document.getElementById("back-in-stock").dataset.variantId);
      backInStockForm.append("platform", "shopify");
      const response = await fetch('https://a.klaviyo.com/onsite/components/back-in-stock/subscribe', {
        body: backInStockForm,
        method: "POST"
      });
      submitButtons.forEach((submitButton) => {
        submitButton.removeAttribute("disabled");
      ...
      const responseJson = await response.json();
      if (response.ok) {
        document.querySelector('.product__back-in-stock-content').style.display = "none"
        document.querySelector('.product__back-in-stock-success').style.display = "block"
      } else {
        document.querySelector('.product__back-in-stock-error').style.display = "block"
      }
    }
  };
  window.customElements.define("back-in-stock-form", BackInStockForm, { extends: "form" });
  ...
  var ProductItem = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.delegate.on("click", ".product-item-meta__swatch-list .color-swatch__item", this._onColorSwatchChanged.bind(this));
    }
    async _onColorSwatchChanged(event, target) {
      if (target.parentNode.classList.contains("active")) return !1; 
      this.querySelectorAll('.color-swatch').forEach((item) => {
        item.classList.remove("active");
      });
      target.parentNode.classList.add("active");
      Array.from(this.querySelectorAll(`[href*="/products"]`)).forEach((link) => {
        link.setAttribute("href", target.getAttribute("data-product-handle"));
      });
      if (JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].image_featured) {
        const newPrimaryImage = JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].image_featured;
        var currentPrimaryImage = this.querySelector(".product-item__primary-image");
        var parentPrimaryImage = currentPrimaryImage.parentNode;
        parentPrimaryImage.removeChild(currentPrimaryImage);
        parentPrimaryImage.innerHTML = newPrimaryImage + parentPrimaryImage.innerHTML;
      };
      if (JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].image_hover) {
        const newSecondaryImage = JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].image_hover;
        var currentSecondaryImage = this.querySelector(".product-item__secondary-image");
        if (currentSecondaryImage) {
          var parentSecondaryImage = currentSecondaryImage.parentNode;
          parentSecondaryImage.removeChild(currentSecondaryImage);
          parentSecondaryImage.innerHTML = parentSecondaryImage.innerHTML + newSecondaryImage;
        }
      }
      this.querySelector(".product-item-meta__title").innerHTML = JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].title
      this.querySelector(".price-list").innerHTML = JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].price
      this.querySelector(".label-list").innerHTML = JSON.parse(this.querySelector("[data-colors-json]").innerHTML)[target.getAttribute("data-product-handle")].sticker
    }
  };
  ...
  // js/custom-element/section/cart/cart-gift.js
  var CartGift = class extends HTMLTextAreaElement {
    connectedCallback() {
      this.addEventListener("change", this._onGiftChanged.bind(this));
    }

    _onGiftChanged(event, target) {
      this._updateFromLink(`${window.themeVariables.routes.cartChangeUrl}?quantity=1&id=${document.querySelector('[data-variant-id="43465471328486"]').dataset.key}`);
    }
    ...
    async _updateFromLink(link) {
      ...
      document.querySelector('[data-variant-id="43465471328486"]').dispatchEvent(new CustomEvent("line-item-quantity:change:start", { bubbles: true, detail: { newLineQuantity: quantity } }));
      const response = await fetch(`${window.themeVariables.routes.cartChangeUrl}.js`, {
        body: JSON.stringify({ id, quantity, properties: properties, sections: ["mini-cart"] }),
        credentials: "same-origin",
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  window.customElements.define("cart-gift", CartGift, { extends: "textarea" });
  ...
  // js/custom-element/section/cart/gift-box.js
  var GiftBox = class extends CustomHTMLElement {
    connectedCallback() {
      this.delegate.on("change", "input", this._onGiftBoxChanged.bind(this));
    }

    _onGiftBoxChanged(event, target) {
      if (target.checked == true) {
        this._createFromLink(`${window.themeVariables.routes.cartChangeUrl}?quantity=1&id=43465471328486`);
      } else {
        this._updateFromLink(`${window.themeVariables.routes.cartChangeUrl}?quantity=0&id=43465471328486`);
      }
    }
    ...
    async _updateFromLink(link) {
      ...
      document.querySelector('[data-variant-id="43465471328486"]').dispatchEvent(new CustomEvent("line-item-quantity:change:start", { bubbles: true, detail: { newLineQuantity: quantity } }));
      const response = await fetch(`${window.themeVariables.routes.cartChangeUrl}.js`, {
        body: JSON.stringify({ id, quantity, sections: ["mini-cart"] }),
        credentials: "same-origin",
        method: "POST",
        headers: {
          "Content-Type": "application/json"
  ...
  // js/custom-element/countdown.js
  var CountDown = class extends HTMLElement {
    connectedCallback() {
      this.querySelector(".counter").innerText = this.querySelector(".counter-input").value.length + ' / ' + this.querySelector(".counter-input").getAttribute('maxlength')
      this.querySelector(".counter-input").addEventListener("keyup", () => {
        this.querySelector(".counter").innerText = this.querySelector(".counter-input").value.length + ' / ' + this.querySelector(".counter-input").getAttribute('maxlength')
      });
  ...
  (() => {
    document.querySelectorAll(".product-item__image-wrapper--multiple").forEach((item) => {
      item.querySelector(".product-item__aspect-ratio").addEventListener("scroll", function() {
        var main = item.querySelector(".product-item__aspect-ratio")
        if (main.scrollLeft > main.offsetWidth / 2) {
          item.querySelector(".slider-steps__item:first-child").classList.remove("active")
          item.querySelector(".slider-steps__item:last-child").classList.add("active")
        } else {
          item.querySelector(".slider-steps__item:first-child").classList.add("active")
          item.querySelector(".slider-steps__item:last-child").classList.remove("active")
        }
      });
    });
  })();
